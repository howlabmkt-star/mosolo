...
export const analyzeMbti = functions.onCall(
  ...
  async (request) => {
    ...
    const result = await callGemini(
      `당신은 MBTI 연애 전문가이자 SNS 바이럴 콘텐츠 크리에이터입니다.
...
export const analyzeMbtiSaju = functions.onCall(
  ...
  async (request) => {
    ...
    const { myMbti, theirMbti, myBirthDate, theirBirthDate, myBirthHour, theirBirthHour } =
      request.data as MbtiSajuRequest;
    ...
    const result = await callGemini(
      `당신은 MBTI 심리학과 동양 사주명리학(四柱命理學)을 결합한 연애 궁합 전문가이자 SNS 바이럴 콘텐츠 크리에이터입니다.
...
export const analyzeSaju = functions.onCall(
  ...
  async (request) => {
    ...
    const { birthDate, birthHour, gender, question } = request.data as SajuRequest;

    if (!birthDate) throw new functions.HttpsError("invalid-argument", "생년월일이 필요합니다");

    const hourStr = birthHour !== undefined ? `출생 시간: ${birthHour}시` : "출생 시간 미입력";
    const questionStr = question ? `\n특별히 궁금한 것: ${question}` : "";

    const result = await callGemini(
      `당신은 동양 사주명리학(四柱命理學) 전문가이자 연애 코치입니다.
